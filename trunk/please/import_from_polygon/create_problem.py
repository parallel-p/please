...
class PolygonImporter:
    ...
    def make_TestInfo(self, i, test, tag):
        ...
        if (test.get('method') == 'manual'):
            ...
            # cross-platform
    #        return FileTestInfo('tests/' + new_test_id), tags)
        else:
            cmd = test.get('cmd').split()
            generator = to_extension[cmd[0]]
            attr = cmd[1:]
    #	    return GeneratorTestInfo(generator, cmd[1:], tags)
    ...
    def make_tests1(self):
        ...
        for testset in self.tree.xpath('/problem/judging/testset'):
            ...
            for i, test in enumerate(testset.xpath('tests/test')):
                test.append(MakeTestInfo(i, test, tag, to_extension))
        return tests
    
    def write_tests1(self, tests):
        with open(os.path.join(cwd, "tests.please"), 'w') as f:
            for test in tests:
                f.write(test.convert_to_please_format() + '\n')
    ...
    def make_tests(self):
        ...
        for testset in self.tree.xpath('/problem/judging/testset'):
            ...
            for i, test in enumerate(testset.xpath('tests/test')):
                if (test.get('method') == 'manual'):
                    test_id = "{0:02d}".format(i + 1) #very bad 0:02d, to think&change
                    new_test_id = tag + '_' + test_id
                    shutil.copy(os.path.join(tag, test_id), \
                                os.path.join(os.path.join(self.cwd, 'tests'), new_test_id))
                    type = 'file'
                    # cross-platform
                    ...
                    type = 'generator'
                    cmd = test.get('cmd').split()
                    command = (self.to_extension[cmd[0]], cmd[1:])
                testinfo = TestInfo()
